...
3. From this repository directory (`HealthApp`), run `npm run dev -- --dev-client` and open the installed **HealthApp Dev** development client on the registered iPhone. For the browser preview, run `npm run web`.
...
The standalone **HealthApp** and Metro-powered **HealthApp Dev** builds can be installed together. The development profile uses `com.jalen.healthapp.dev` and `healthapp-dev`; preview and production retain `com.jalen.healthapp` and `healthapp`. Build the development client from `apps/mobile` with `npx eas-cli@latest build --platform ios --profile development`. The normal root `npm run dev -- --dev-client` command automatically starts Expo with the development variant.
