...
export async function cachedVitals(userId: string): Promise<VitalSample[]> {
  ...
  const rows = await db.getAllAsync<Record<string, unknown>>(
    "SELECT * FROM vital_samples WHERE user_id = ? ORDER BY occurred_at DESC",
    [userId],
  );
...
export async function saveCachedVitals(samples: VitalSample[]): Promise<void> {
  ...
  await db.withTransactionAsync(async () => {
    for (const sample of samples) {
      await db.runAsync(
        `INSERT INTO vital_samples (id, user_id, kind, value, unit, occurred_at, correlation_id, source, external_id, source_name, created_at, deleted_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ON CONFLICT(id) DO UPDATE SET value = excluded.value, unit = excluded.unit,
          ...
          source_name = excluded.source_name, deleted_at = excluded.deleted_at`,
        [
          sample.id,
          sample.userId,
          sample.kind,
...
export async function enqueueVitals(samples: VitalSample[]): Promise<void> {
  const db = await database();
  await db.runAsync(
    "INSERT INTO outbox (id, operation, payload, created_at) VALUES (?, ?, ?, ?)",
    [
      createId(),
      "upsert_vitals",
...
export async function queuedChanges(): Promise<OutboxRecord[]> {
  const db = await database();
  return db.getAllAsync<OutboxRecord>(
    "SELECT id, operation, payload, attempts FROM outbox ORDER BY created_at ASC",
  );
}
...
export async function lastVitalSyncAt(
  userId: string,
...
export async function saveVitalSyncAt(
  userId: string,
  value: string,
): Promise<void> {
  const db = await database();
