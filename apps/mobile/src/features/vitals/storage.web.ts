...
export async function cachedVitals(userId: string): Promise<VitalSample[]> {
  return (await read<VitalSample[]>(samplesKey, []))
...
export async function saveCachedVitals(samples: VitalSample[]): Promise<void> {
  ...
  const merged = existing.map(
    (sample) => replacements.get(sample.id) ?? sample,
  );
  for (const sample of samples)
    if (!existing.some((item) => item.id === sample.id)) merged.push(sample);
...
export async function completeChange(id: string): Promise<void> {
  await write(
    outboxKey,
    (await queuedChanges()).filter((change) => change.id !== id),
  );
}
...
export async function failChange(id: string): Promise<void> {
  await write(
    outboxKey,
    (await queuedChanges()).map((change) =>
      change.id === id ? { ...change, attempts: change.attempts + 1 } : change,
    ),
  );
}
...
export async function lastVitalSyncAt(
  ...
  try {
    return (await AsyncStorage.getItem(syncKey(userId))) ?? undefined;
  } catch {
    return undefined;
  }
}
export async function saveVitalSyncAt(
  ...
  await AsyncStorage.setItem(syncKey(userId), value);
}
