...
- Name search: **Find or add food** searches only the signed-in user's Recent and My Foods records. An unmatched non-empty name offers a prefilled reusable label; exact saved-name matches are selected instead of creating another quick entry.
...
- Serving behavior: named household servings such as package, piece, bar, or slice remain distinct from exact mass/volume conversions. Structured `100g` and `100ml` bases map to mass and volume respectively. Conflicting OFF input sets do not produce an inferred package size; their serving conversions remain blank until the user confirms the package label.
