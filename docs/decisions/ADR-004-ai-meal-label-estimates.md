...
Validate strict structured output on both sides. Use a reproducible one-serving nutrient basis plus a distinct consumed amount so the existing portion calculator remains authoritative. Countable foods use a singular household unit and per-item serving, so eight dumplings remain editable as `8 dumplings`; other foods include estimated serving weight or volume. Users review/edit/remove components before accepting them into their existing meal draft. Each reviewed food defaults to a private reusable profile, but users can turn that choice off independently and log only that food's immutable meal-history snapshot. Label descriptions and AI source provenance survive persistence when reusable profiles are created.
...
- The label-saving choice is explicit for each reviewed food. Turning one off prevents both immediate and final-save profile creation for that item while retaining its reviewed nutrition in meal history.
- Estimation instructions include a conservative cooking-fat assumption when the described preparation normally uses oil or butter, except when the user explicitly indicates dry, oil-free, air-fried, boiled, or steamed preparation. The assumption remains visible to the user.
