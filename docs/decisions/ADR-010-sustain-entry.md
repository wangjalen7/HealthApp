...
- `authenticating` belongs to a numbered manual, biometric, unlock or restoration attempt. Password and Face ID sign-in use one centered opaque entry cover from the pending request onward. The covered form stays static and inaccessible. The approved logo uses the same contained highlight for both pending and biometric states, around the native Face ID prompt. Synchronous submission guards prevent duplicate requests.
...
- `entering` starts after the internal destination is selected; routing occurs behind an opaque cover. The cover fades only once the destination path is active and access checks still pass. Password and Face ID sign-in use a 320ms fill/pulse and 200ms fade; existing-session unlock retains its 180ms plus 140ms transition. Session-only restoration uses only a short fade. Reduce Motion removes the motion and success haptic. Supported iPhones get at most one light haptic per attempt.
- The sign-in symbol is centered with a responsive maximum width of 144 points; the existing unlock remains 112 points. Keyboard dismissal prevents the old form inset from shifting the cover. A ready-route-only completion timer prevents a stalled animation callback from trapping an already-authorized route; it is canceled if readiness changes.
...
The Face ID credential exchange waits for iOS to report `active` after saving the rotated Keychain credential and before installing the Supabase session. Native authentication can return while the app is still `inactive`; installing the session then triggers the normal privacy lock and invalidates entry, stranding the login form. An event-driven foreground handoff avoids bypassing the privacy lock or adding an arbitrary success delay. Actual backgrounding or a missing foreground event for ten seconds aborts the handoff with retry feedback, removes its listener, and retains the rotated credential for the next attempt. Later app departure still locks normally. This race is covered with the real credential helper and AuthProvider using synthetic native adapters; real-device timing remains a device check.
...
The original entry implementation added two Expo SDK 57 modules, splash-screen and haptics. The splash configuration and native modules require a new iOS binary; reloading an older development client is insufficient. Expo documents that development clients do not fully reproduce standalone splash behavior, so final launch continuity must be checked in a standalone build. Existing bundle identifiers, auth callback schemes and deployment identity remain unchanged. The subsequent centered-entry/loading refinement adds no native dependency or native configuration change.
