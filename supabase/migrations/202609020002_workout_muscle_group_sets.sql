-- Persist the muscle group for each exercise so workout history can show exact set totals by group.

alter table public.workout_sets
  add column muscle_group text
    check (muscle_group is null or muscle_group in ('Back', 'Chest', 'Tri', 'Bi', 'Delt', 'Legs', 'Abs'));

update public.workout_sets as workout_set
set muscle_group = session.muscle_groups[1]
from public.workout_sessions as session
where workout_set.session_id = session.id
  and cardinality(session.muscle_groups) = 1
  and session.muscle_groups[1] in ('Back', 'Chest', 'Tri', 'Bi', 'Delt', 'Legs', 'Abs');
